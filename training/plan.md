...
系统性地教各种知识一是慢，二是他们能够消化的少，三是他们学到后可能会认为没有意义。因此，课程安排以问题导向和实践目标驱动为主。
...
- 机器人整体系统概论：组成、技术组工作。
- 建议面向所有人；可参考负责人：张雨轩 / 刘义。分开讲还是一起讲，可根据实际安排确定。
...
- 计算机基础：git、命令行、了解操作系统（0）。
    - 可补充：计算机硬件介绍、操作系统、Linux 介绍和基本使用、什么是命令行、安装 Ubuntu。
    - Git 代码管理。
- 科学上网（-0）。
- 如何正确使用 AI（0）。
    - 可补充：如何使用 AI 编程和学习，可线上。
- 提问的艺术（0）。
...
- 课程设计补充：
    - 安装 Keil、下载标准库文件（线上）。
    - 计算机硬件介绍、嵌入式是什么、实现 GPIO（1 课时）。
    - TIM、PWM 和中断，使用调试（1 课时）。
    - 安装 CubeMX 和 Keil（线上）。
    - 转 HAL 库，串口和中断（1 课时）。
- 进阶内容：
    - CAN 通信。
...
- 课时数沿用策划书，0 为线上课。
- 课程设计中的详细内容用于填充课程细节；若与策划书冲突，以策划书为准。
- 负号项为原稿标记，具体排课时再确认。